...
fun <T> dataAccess(networkCall: suspend () -> NetworkResponse<T>): LiveData<NetworkResponse<T>> = liveData(
    ...
    if (responseStatus.status == NetworkResponse.Status.SUCCESS) {
        emit(NetworkResponse.success(responseStatus.code,responseStatus.data, responseStatus.headers) as NetworkResponse<T>)

    } else if (responseStatus.status == NetworkResponse.Status.ERROR) {
        emit(NetworkResponse.error(responseStatus.code,responseStatus.message!!,responseStatus.data,responseStatus.error))
    }
}
